...
EXPORT void build_momentum_rhs_u2(
      Array1<double> momentum_rhside_u2,			// momentum matrix velocity x1 direction
       ...
       )
  {
	  ...
	  for(j_index=0;j_index<number_primary_cells_j+1;j_index++)
	  {
		  ...
		  level_set_face_x2_min=level_set[i_index  ][j_index  ][k_index  ];
		  level_set_face_x2_pls=level_set[i_index  ][j_index+1][k_index  ];
		  ...
		  /* evaluate the density at the center of the control volume */
                ...
                density_cell_center=scaled_density_u2[i_index][j_index][k_index];
...
//                                     rho_plus_over_rho_minus);
		  ...
		  u_1_velocity_cell_center=0.25*(
					   ...
					   u_1_velocity_old[i_index  ][j_index+1][k_index  ]+
					   u_1_velocity_old[i_index-1][j_index  ][k_index  ]+
					   u_1_velocity_old[i_index-1][j_index+1][k_index  ]
					     );
		  u_2_velocity_cell_center=u_2_velocity_old[i_index][j_index][k_index];
		  u_3_velocity_cell_center=0.25*(
					   ...
					   u_3_velocity_old[i_index  ][j_index+1][k_index-1]
					     );
		  if(j_index==0)
		  {
			      ...
			      compute_convection_term(u_1_velocity_cell_center,
							u_2_velocity_cell_center,
							  u_3_velocity_cell_center,
							      ...
							      u_2_velocity_old[i_index-1][j_index  ][k_index  ],
							      u_2_velocity_old[i_index+1][j_index  ][k_index  ],
							      u_2_velocity_old[i_index  ][j_index  ][k_index  ],
							      u_2_velocity_old[i_index  ][j_index+1][k_index  ],
							      u_2_velocity_old[i_index  ][j_index  ][k_index-1],
							      u_2_velocity_old[i_index  ][j_index  ][k_index+1],
							      mesh_width_x1, 2*mesh_width_x2, mesh_width_x3	
 						    );
		  }
		  else
		  {
		  ...
		  diffusion_term=
				 one_over_dx1*(
				   viscosity_x1_pls*one_over_dx2*(
				      u_1_velocity_old[i_index  ][j_index+1][k_index  ]-
					u_1_velocity_old[i_index  ][j_index  ][k_index  ]) -
					
				   viscosity_x1_min*one_over_dx2*(
				      u_1_velocity_old[i_index-1][j_index+1][k_index  ]-
					u_1_velocity_old[i_index-1][j_index  ][k_index  ]))-
					
				 one_over_dx2*(
				   viscosity_x2_pls*one_over_dx1*(
				      u_1_velocity_old[i_index  ][j_index+1][k_index  ]-
					u_1_velocity_old[i_index-1][j_index+1][k_index  ]) -
					
				   viscosity_x2_min*one_over_dx1*(
				      u_1_velocity_old[i_index  ][j_index  ][k_index  ]-
					u_1_velocity_old[i_index-1][j_index  ][k_index  ]))+		
		  ...
		  one_dimensional_index=map_index_u2(i_index,j_index,k_index,
			...
			density_cell_center*u_2_velocity_old[i_index][j_index][k_index]+
			  actual_time_step_navier_stokes*(-1.0*convection_term+
				    diffusion_term + 
				      density_cell_center*momentum_source_term_u_2[i_index][j_index][k_index]);
