...
{
      int map_index_u1(                    		// map 3-D array index to 1-D array
	  int i_index,					// index
	  int j_index, 				
	  int k_index,  			
   ...
   /* start the loop over all faces for the matrix folding */
	  ...
	  if(boundary_faces[face_index].boundary_variables[0].boundary_condition_type==dirichlet)
	  {
		 ...
		 for(j_index=1;j_index<number_primary_cells_j+1;j_index++)
		 {
		      ...
		      {
			/* U_real=U_boundary */
 			  one_dimensional_index=map_index_u1(cell_label_boundary,j_index,k_index,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);
			  momentum_matrix_u1[0][one_dimensional_index]=1.0;
			  for(connection_index=1;connection_index<number_matrix_connections;
							      connection_index++)
			  {
			      momentum_matrix_u1[connection_index][one_dimensional_index]=0.0;
			  }
		      }	  
		 ...
		 /* only for the face 0, the other non existing references */
		 ...
		 for(j_index=1;j_index<number_primary_cells_j+1;j_index++)
		 {
		      ...
		      {
			/* U_real=U_boundary */
 			  one_dimensional_index=map_index_u1(cell_label_boundary-1,j_index,k_index,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);
			  momentum_matrix_u1[1][one_dimensional_index]=0.0;
		      }	  
	  ...
	  else
	  {
	      ...
	      std::cerr<<"in function fold_momentum_matrix_u1 line 155 \n";
	      std::cerr<<"**************************************************** \n";
	      exit(1);
	  }
	  ...
	  if(boundary_faces[face_index].boundary_variables[0].boundary_condition_type==dirichlet)
	  {
		...
		if(face_index==2){
			...
			/* and the nonexistent connection is set to zero */
			  one_dimensional_index=map_index_u1(i_index,cell_label_boundary,k_index,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);

			    momentum_matrix_u1[0][one_dimensional_index]-=
			    momentum_matrix_u1[2][one_dimensional_index];
			    momentum_matrix_u1[2][one_dimensional_index]=0;
	  ...
	  else
	  {
		...
		else
		{
		  ...
		  std::cerr<<"you are trying to impose a boundary condition that \n";
		  std::cerr<<"is not implemented yet, check your input";
		  std::cerr<<"in function fold_momentum_matrix_u1 line 277 \n";
		  std::cerr<<"**************************************************** \n";
		  exit(1);
	      }
	  }
	  ...
	  if(boundary_faces[face_index].boundary_variables[0].boundary_condition_type==dirichlet)
	  {
		/* DIRICHLET BOUNDARY CONDITION */
		 ...
		 if(face_index==4)
		 {
			...
			/* coefficient with opposite sign */
			/* and the nonexistent connection is set to zero */
			  one_dimensional_index=map_index_u1(i_index,j_index,cell_label_boundary,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);

			    momentum_matrix_u1[0][one_dimensional_index]-=
			    momentum_matrix_u1[3][one_dimensional_index];
			    momentum_matrix_u1[3][one_dimensional_index]=0;
			  }	  
	  ...
	  else
	  {
		 ...
		 else
		 {
		  ...
		  std::cerr<<"you are trying to impose a boundary condition that \n";
		  std::cerr<<"is not implemented yet, check your input";
		  std::cerr<<"in function fold_momentum_matrix_u1 line 397 \n";
		  std::cerr<<"**************************************************** \n";
		  exit(1);
	      }
	  }
