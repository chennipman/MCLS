   ...
   /* start the loop over all faces for the matrix folding */
	  ...
	  if(boundary_faces[face_index].boundary_variables[1].boundary_condition_type==dirichlet)
	  {
		...
		if(face_index==0)
		{
			...
			/* and the nonexistent connection is set to zero */
			    ...
			    momentum_matrix_u2[1][one_dimensional_index]=0;
		...
		if(face_index==2)
		{
			...
			for(k_index=1;k_index<number_primary_cells_k+1;k_index++)
			{
			    ...
			    one_dimensional_index=map_index_u2(i_index,cell_label_boundary,k_index,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);
			    momentum_matrix_u2[0][one_dimensional_index]=1.0;
			    for(connection_index=1;connection_index<number_matrix_connections;
							      connection_index++)
			    {
				momentum_matrix_u2[connection_index][one_dimensional_index]=0.0;
			    }
			}	  
		 ...
		 /* only for the face 2, the other non existing references */
		 /* are in the lower triangular part of the matrix	   */
			...
			for(k_index=1;k_index<number_primary_cells_k+1;k_index++)
			{
			    ...
			    one_dimensional_index=map_index_u2(i_index,cell_label_boundary-1,k_index,
				      number_primary_cells_i, number_primary_cells_j, 
								  number_primary_cells_k);
			    momentum_matrix_u2[2][one_dimensional_index]=0.0;
			}	  
	  ...
	  else
	  {
	   std::cout<< "this is not implemented yet, check your input, aborting...";
		  ...
		  std::cerr<<"you are trying to impose a boundary condition that \n";
		  std::cerr<<"is not implemented yet, check your input";
		  std::cerr<<"in function fold_momentum_matrix_u1 line 282 \n";
		  std::cerr<<"**************************************************** \n";
	   exit(1);
	  }
		...
		/* DIRICHLET BOUNDARY CONDITION */
		 ...
		 if(face_index==4)
		 {
			...
			/* and the nonexistent connection is set to zero */
			    ...
			    momentum_matrix_u2[3][one_dimensional_index]=0;
			  }	  
	  ...
	  else
	  {
		 ...
		 else
		 {
		  ...
		  std::cerr<<"you are trying to impose a boundary condition that \n";
		  std::cerr<<"is not implemented yet, check your input";
		  std::cerr<<"in function fold_momentum_matrix_u1 line 403 \n";
		  std::cerr<<"**************************************************** \n";
		  exit(1);
	      }
	  }
