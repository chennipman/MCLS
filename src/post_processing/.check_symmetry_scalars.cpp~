...
/********************************************************************************/
    void check_symmetry_scalars(
      ...
      double ***field					// field of which symmetry must be checked
	  )
    {
      ...
      double max_error_i_rot=0;				// max difference between i and j constant planes
      double max_error_j_rot=0;				// max difference between i and k constant planes
      double max_error_k_rot=0;				// max difference between j and j constant planes
		...
		for(j_index=1;j_index<number_primary_cells_j+1;j_index++)
			...
			field[i_index][symmetry_plane_index][j_index]-
				field[i_index][j_index][symmetry_plane_index]),
			    		max_error_jk);
			max_error_i_rot=std::max(fabs(
			field[symmetry_plane_index][i_index][j_index]-
				field[symmetry_plane_index][j_index][i_index]),
			    		max_error_i_rot);
			max_error_j_rot=std::max(fabs(
			field[i_index][symmetry_plane_index][j_index]-
				field[j_index][symmetry_plane_index][i_index]),
			    		max_error_j_rot);
			max_error_k_rot=std::max(fabs(
			field[i_index][j_index][symmetry_plane_index]-
				field[j_index][i_index][symmetry_plane_index]),
			    		max_error_k_rot);
            	}
          }
          ...
          std::cerr<<"max error i_rot "<< max_error_i_rot <<"\n";
          std::cerr<<"max error j_rot "<< max_error_j_rot <<"\n";
          std::cerr<<"max error k_rot "<< max_error_k_rot <<"\n";
    }     