...
/* if the testcase is two-dimensional, with one of the grid dimensions equal to unity 	     */
/* the whole solution is simply copied to the other slices, that comprise the virtual cells */
/* then the whole normal procedure can be followed						     */

	/* the flow is two-dimensional with nonresolved direction x1 */
	if(number_primary_cells_i==1){
		  ...
		  {
		      field[0][j_index][k_index] =field[1][j_index][k_index] ;
		  ...
		  for(k_index=0;k_index<number_primary_cells_k+2;k_index++)
		  {
		      field[2][j_index][k_index] =field[1][j_index][k_index] ;

		  }  
	      }  
	}
	
	/* the flow is two-dimensional with nonresolved direction x2 */
	if(number_primary_cells_j==1){
	      for(i_index=0;i_index<number_primary_cells_i+2;i_index++)
	      {
		  for(k_index=0;k_index<number_primary_cells_k+2;k_index++)
		  {
		      field[i_index][0][k_index] =field[i_index][1][k_index];
		  ...
		  for(k_index=0;k_index<number_primary_cells_k+2;k_index++)
		  {
		      field[i_index][2][k_index] =field[i_index][1][k_index];

		  }  
	      }  
	}
	
	/* the flow is two-dimensional with nonresolved direction x3 */
	if(number_primary_cells_k==1){
	      for(i_index=0;i_index<number_primary_cells_i+2;i_index++)
	      {
		  for(j_index=0;j_index<number_primary_cells_j+2;j_index++)
		  {
		      field[i_index][j_index][0] =field[i_index][j_index][1] ;
		  ...
		  for(j_index=0;j_index<number_primary_cells_j+2;j_index++)
		  {
		      field[i_index][j_index][2] =field[i_index][j_index][1] ;
...
/* and the higher index virtual cell has the value of the last real cell */
	  ...
	  for(i_index=i_index_start;i_index<i_index_end+1;i_index++)
	  {
		  ...
		  for(k_index=k_index_start;k_index<k_index_end+1;k_index++)
		  {
		      field[i_index+i_shift][j_index+j_shift][k_index+k_shift] =  
				      2*field[i_index][j_index][k_index]-
					field[i_index-i_shift][j_index-j_shift][k_index-k_shift];
      ...
      field[number_primary_cells_i+1][0][0]				=
			    2*field[number_primary_cells_i][1][1]-
				  field[number_primary_cells_i-1][2][2];
      field[number_primary_cells_i+1][number_primary_cells_j+1][0]	=
			    2*field[number_primary_cells_i][number_primary_cells_j][1]-
				  field[number_primary_cells_i-1][number_primary_cells_j-1][2];
      field[0][number_primary_cells_j+1][0]				=
			    2*field[1][number_primary_cells_j][1]-
				  field[2][number_primary_cells_j-1][2];
      field[0][0][number_primary_cells_k+1]				=
			    2*field[1][1][number_primary_cells_k]-
				  field[2][2][number_primary_cells_k-1];
      field[number_primary_cells_i+1][0][number_primary_cells_k+1]	=
			    2*field[number_primary_cells_i][1][number_primary_cells_k]-
				  field[number_primary_cells_i-1][2][number_primary_cells_k-1];
      field[number_primary_cells_i+1][number_primary_cells_j+1][number_primary_cells_k+1]	=
			    2*field[number_primary_cells_i][number_primary_cells_j][number_primary_cells_k]-
				  field[number_primary_cells_i-1][number_primary_cells_j-1][number_primary_cells_k-1];
      field[0][number_primary_cells_j+1][number_primary_cells_k+1]				=
			    2*field[1][number_primary_cells_j][number_primary_cells_k]-
				  field[2][number_primary_cells_j-1][number_primary_cells_k-1];
